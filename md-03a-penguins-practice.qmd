...
5.  What data types are contained in the variables? categorical (species, island sex), continuous (bill length/depth, flipper length, body mass), discrete (year)
...
penguins %>%
  ...
  summarise(flip_max = max(flipper_length_mm, na.rm=TRUE),
            flip_min = min(flipper_length_mm, na.rm = TRUE)) #remove NA values
...
penguins %>%
  ...
  summarise(bill_depth_mean = mean(bill_depth_mm, na.rm=TRUE),
            bill_length_mean = mean(bill_length_mm, na.rm = TRUE)) #remove NA values
